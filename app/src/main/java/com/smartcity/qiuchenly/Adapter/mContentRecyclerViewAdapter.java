...
public class mContentRecyclerViewAdapter extends
    ...
    public mContentRecyclerViewAdapter(userManageModel lists, iController.iCarPayment carPayment) {
        this.lists = lists;
    ...
    public void onBindViewHolder(final VH h, final int p) {
        final userManageModel.datas user = lists.data[p];
        h.index.setText(p + 1 + "");//fix INDEX bug
        h.payMent.setText("充值");
        h.payMent.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                carPayment.wantPaymentCarID(user.carID, user.carMaster, user.totalMoney);
            }
        });
